...
- **プロジェクト理解**: Simutrans の歴史、目標、技術的背景を把握
- **技術スタック把握**: 使用言語、ライブラリ、ビルドシステムの全体像
- **開発開始の準備**: 開発環境構築前に必要な情報を提供
- **アーキテクチャ概要**: プロジェクト構造とコンポーネントの関係を理解

**対象読者:**

- 初めて Simutrans のコードに触れる開発者
- プロジェクト構造を理解したい方
- 技術スタックの全体像を把握したい方
...
- **CMake**: `CMakeLists.txt`, `cmake/Simutrans*.cmake`
- **Make**: `Makefile`, `common.mk`, `uncommon.mk`
- **Visual Studio**: `Simutrans.sln`, `Simutrans-*.vcxproj`
...
- **オープンソース**: Artistic License 1.0 で自由に貢献可能
- **クロスプラットフォーム**: Windows、Linux、macOS、Android に対応
- **柔軟なビルドシステム**: Make、CMake、Visual Studio をサポート
- **モジュラー設計**: グラフィックバックエンド、ネットワーク、AI が独立
- **活発なコミュニティ**: フォーラム、GitHub で活発な開発が継続中

このプロジェクト概要を理解したら、次は以下のドキュメントに進みましょう：

1. [DEVELOPMENT_SETUP.md](DEVELOPMENT_SETUP.md) - 開発環境の構築
2. [DEPENDENCIES.md](DEPENDENCIES.md) - 依存ライブラリの詳細
3. [TECHNICAL_ARCHITECTURE.md](../architecture/TECHNICAL_ARCHITECTURE.md) - アーキテクチャの深掘り
